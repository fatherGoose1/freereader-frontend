...
export type KokoroWebRequest = { text: string; voice: KokoroVoice; speechSpeed: number };
...
export class KokoroWebSpeechClient {
  ...
  synthesize(text: string, voice: KokoroVoice, speechSpeed: number, status?: TtsStatus) {
    ...
    const task = this.tail.then(() => new Promise<{ blob: Blob; duration: number; provider: string; generationSeconds: number }>((resolve, reject) => {
      ...
      try {
        ...
        watchdog();
        this.worker.postMessage({ text, voice, speechSpeed } satisfies KokoroWebRequest);
