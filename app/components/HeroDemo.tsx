    ...
    <div className="hero-demo">
      <div className="hero-demo-head">
        ...
        <div className={`hero-demo-language-menu${languagesOpen ? " is-open" : ""}`}>
          <button
            type="button"
            className="hero-demo-tag"
            aria-label="Supported Languages"
            aria-expanded={languagesOpen}
            aria-controls="hero-demo-languages"
            onClick={() => setLanguagesOpen((open) => !open)}
          >
            {SPEECH_LANGUAGES.length} languages
          </button>
          <div id="hero-demo-languages" className="hero-demo-language-popover">
            <ul>
              {SORTED_LANGUAGES.map(([code, name]) => <li key={code}>{name}</li>)}
            </ul>
          </div>
        </div>
      </div>
      ...
      <p className="hero-demo-note"><span aria-hidden="true">●</span> Language detected automatically</p>
    </div>
  );
}
